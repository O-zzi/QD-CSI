...
-- First, check the cms_content table structure
-- SELECT column_name FROM information_schema.columns WHERE table_name = 'cms_content';
...
-- If Terms/Privacy don't exist, insert them:
INSERT INTO cms_content (id, key, title, content, is_active) 
SELECT gen_random_uuid()::text, 'privacy_policy', 'Privacy Policy', '[
...
WHERE NOT EXISTS (SELECT 1 FROM cms_content WHERE key = 'privacy_policy');

INSERT INTO cms_content (id, key, title, content, is_active) 
SELECT gen_random_uuid()::text, 'terms_conditions', 'Terms & Conditions', '[
...
WHERE NOT EXISTS (SELECT 1 FROM cms_content WHERE key = 'terms_conditions');
...
UNION ALL
SELECT 'CMS Content' as data_type, COUNT(*) as count FROM cms_content WHERE is_active = true;
